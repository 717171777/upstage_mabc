...
function ExportWorkspace({job}:{job:Job}) {
  ...
  return <ServiceShell step={3} title="비교하고 내보내기">
    ...
    <div className="mt-5 space-y-3">
      ...
      {validated&&<section className="rounded-xl border border-slate-200 bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-sm font-semibold">가린 사본 AI 검토</h2><p className="mt-1 text-xs text-slate-500">남아 있는 정보와 누락 의심 항목을 추가로 확인합니다.</p></div><button onClick={reviewCopy} disabled={disabled||!job.aiEnabled||reportRunning} className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm text-blue-700 disabled:opacity-40">{reportRunning?'AI 검토 중…':report?'다시 AI 검토':'AI 검토 시작'}</button></div><p className="mt-3 text-xs text-slate-400">Upstage에 저장 사본, Hermes·SP4에 필요한 근거를 보냅니다.</p>
