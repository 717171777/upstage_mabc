...
function ExportWorkspace({job}:{job:Job}) {
  ...
  const allReady=!!job&&isUpstageComplete(job)&&job.candidates.every(c=>c.confirmed&&c.locationResolved)&&!job.analysis.incomplete&&!(job.fullRedaction&&job.uninspected.length)&&!['analyzing','rendering'].includes(job.status);
