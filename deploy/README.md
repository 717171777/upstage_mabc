...
- ANTHROPIC_API_KEY도 서버의 비밀 환경 변수로 설정한다. GARIMI_CLAUDE_MODEL=claude-sonnet-5로 고정한다. Upstage 키만으로는 AI 연결 준비 상태가 되지 않는다.
...
docker run --name garimi --init -p 127.0.0.1:3002:3000 \
  ...
  -e UPSTAGE_API_KEY \
  -e ANTHROPIC_API_KEY \
  -e GARIMI_CLAUDE_MODEL=claude-sonnet-5 \
  garimi:release
```
...
- 일반 문서도 Parse·Classify·Information Extract를 거친다. 필요한 원문 근거·위치·공유 상황만 Hermes·Claude 예외 판단에 보낸다. 소스 코드·개발 지시는 운영 요청에 넣지 않는다.
